...
export function ImageUploadDialog({ isOpen, onClose, testType, testTitle }: ImageUploadDialogProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // For diabetic retinopathy, call the API
      if (testType === "diabetic-retinopathy") {
        try {
          ...
          console.error('API call failed, using mock data for demo:', apiError);
          // Mock response for demo purposes when API is not accessible
          analysisResult = {
            "class_name": "Moderate",
            "class_id": 2,
            "confidence": 0.5477659106254578
          };
          setApiResponse(analysisResult);
        }
      }
      ...
      // Map API response to risk level and convert test type
      const getRiskLevel = (className: string) => {
        switch (className?.toLowerCase()) {
          case 'no dr': return 'low';
          case 'mild': return 'low';
          case 'moderate': return 'moderate';
          case 'severe': return 'high';
          case 'proliferative dr': return 'high';
          default: return 'low';
        }
      };

      // Convert test type to match database constraint
      ...
      const { error: dbError } = await supabase
        ...
        .insert({
          ...
          risk_level: analysisResult ? getRiskLevel(analysisResult.class_name) : 'low',
        ...
        <div className="space-y-6">
          ...
          <Card className="bg-primary/5 border-primary/20">
            <CardContent className="p-4">
              <h4 className="font-semibold text-primary mb-2">Upload Instructions</h4>
              ...
              {apiResponse && (
                <Card className="bg-muted/50">
                  <CardContent className="p-6">
                    <h4 className="font-semibold mb-4">Diabetic Retinopathy Analysis Results</h4>
                    <div className="space-y-4">
                      <div className="flex items-center justify-between p-4 bg-background rounded-lg border">
                        <div>
                          <h5 className="font-medium text-lg">Classification</h5>
                          <p className="text-2xl font-bold text-primary">{apiResponse.class_name}</p>
                        </div>
                        <div className="text-right">
                          <h5 className="font-medium text-sm text-muted-foreground">Confidence Level</h5>
                          <p className="text-xl font-semibold">{(apiResponse.confidence * 100).toFixed(1)}%</p>
                        </div>
                      </div>
                      
                      <div className="bg-background p-4 rounded-lg border">
                        <h5 className="font-medium mb-2">Classification Details</h5>
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          <div>
                            <span className="text-muted-foreground">Severity Level:</span>
                            <span className="ml-2 font-medium">{apiResponse.class_name}</span>
                          </div>
                          <div>
                            <span className="text-muted-foreground">Class ID:</span>
                            <span className="ml-2 font-medium">{apiResponse.class_id}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="bg-primary/5 border border-primary/20 p-4 rounded-lg">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <h5 className="font-medium text-primary mb-1">Next Steps</h5>
                            <p className="text-sm text-muted-foreground">
            ...
            <CardContent className="p-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    This AI analysis is for screening purposes only and does not replace professional medical diagnosis.
                    Always consult with an eye care professional for comprehensive evaluation.
                  </p>
                </div>
              </div>
            </CardContent>
